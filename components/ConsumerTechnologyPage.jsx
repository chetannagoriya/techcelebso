  ...
  return (
    <main className="bg-[#fbfcff] text-[#10172f] dark:bg-[#071126] dark:text-white">
      <section className="relative min-h-[610px] overflow-hidden bg-[#020a1d] text-white">
        ...
        <div className="relative mx-auto flex min-h-[610px] max-w-[1440px] items-center px-6 py-14 md:px-10 lg:px-14">
          <div className="max-w-[620px]">
            <span className="rounded border border-[#00F5D4]/25 bg-[#00F5D4]/10 px-3 py-1.5 text-[10px] font-bold tracking-wider text-[#00F5D4]">CONSUMER TECHNOLOGY</span>
            <h1 className="mt-6 text-4xl font-extrabold leading-[1.08] tracking-[-.04em] sm:text-5xl lg:text-[52px]">Powering the Future of <span className="block bg-gradient-to-r from-[#00F5D4] to-[#00A98F] bg-clip-text text-transparent">Consumer Technology</span></h1>
            ...
            <div className="mt-7 flex flex-wrap gap-3">
              <Link href="/contact" className="inline-flex items-center gap-2 rounded-md bg-[#00A98F] px-5 py-3 text-xs font-bold transition hover:bg-[#00D9BD]">Book Free Consultation <ArrowRight size={14}/></Link>
            </div>
            <div className="industry-hero-icon-strip mt-11 grid grid-cols-3 gap-5 sm:grid-cols-6" style={{ position: "absolute", bottom: 0, left: "50%", zIndex: 20, width: "min(1200px, calc(100% - 3rem))", minHeight: 78, margin: 0, transform: "translateX(-50%)" }}>
              {[[Palette,"User-Centric Design"],[Boxes,"Scalable Architecture"],[Sparkles,"AI-First Approach"],[Cloud,"Cloud Native"],[LockKeyhole,"Secure & Compliant"],[TrendingUp,"Data Driven Insights"]].map(([Icon,label]) => <div key={label} className="text-center"><Icon className="mx-auto text-[#00F5D4]" size={22}/><span className="mt-2 block text-[8px] font-semibold leading-3 text-white/75">{label}</span></div>)}
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="mx-auto max-w-[1360px] px-6 py-14 md:px-10">
        ...
        <div className="grid gap-5 lg:grid-cols-3">
          {solutions.map(({title,icon:Icon,image,copy,items}) => <article key={title} className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
            ...
            <div className="p-5"><div className="flex items-center gap-3"><span className="grid h-9 w-9 place-items-center rounded-lg bg-[#00A98F] text-white"><Icon size={18}/></span><h3 className="text-base font-bold">{title}</h3></div><p className="mt-4 text-[11px] leading-5 text-slate-600">{copy}</p><ul className="mt-4 space-y-2 text-[10px] text-slate-700">{items.map(item=><li key={item} className="flex items-center gap-2"><Check size={12} className="text-[#00A98F]"/>{item}</li>)}</ul><Link href="/contact" className="mt-5 inline-flex items-center gap-1 text-[10px] font-bold text-[#008D7A]">Learn More <ArrowRight size={12}/></Link></div>
          </article>)}
        </div>
      </section>

      <section className="mx-auto max-w-[1440px] px-6 pb-12 md:px-10"><SectionTitle>Use Cases</SectionTitle><UseCasesMarquee useCases={useCases} label="Consumer technology use cases"/></section>
      ...
      <section className="mx-auto grid max-w-[1360px] gap-5 px-6 pb-12 md:px-10 lg:grid-cols-[.78fr_1.22fr]">
        ...
        <div className="relative min-h-[330px] overflow-hidden rounded-xl bg-[#06102c] text-white"><Image src="/images/consumer-technology/cta.png" alt="Product designer building a consumer technology experience" fill className="object-cover object-center"/><div className="absolute inset-0 bg-gradient-to-r from-[#06102c] via-[#06102c]/85 to-transparent"/><div className="relative max-w-md p-8"><h2 className="text-3xl font-bold">Ready to Build the Next Big Thing?</h2><p className="mt-3 text-[11px] leading-5 text-white/75">Let&apos;s build powerful consumer technology products that your users will love.</p><div className="mt-6 flex flex-wrap gap-3"><Link href="/contact" className="rounded-md bg-[#00A98F] px-5 py-3 text-[10px] font-bold">Book Free Consultation</Link></div></div></div>
      </section>
    </main>
  );
}
